...
public class DungeonManager : MonoBehaviour {
	...
	private void BuildRandomPath () {
		...
		Queue<PathTile> pathQueue = new Queue<PathTile>();
		foreach (KeyValuePair<Vector2,TileType> tile in gridPositions) {
			Vector2 tilePos = new Vector2(tile.Key.x, tile.Key.y);
			pathQueue.Enqueue(new PathTile(TileType.random, tilePos, minBound, maxBound, gridPositions));
		}


		while (pathQueue.Count > 0)
		{
			PathTile tile = pathQueue.Dequeue();
			...
			if (adjacentTileCount != 0) {
				if (Random.Range(0, 5) == 1) {
					...
					if (!gridPositions.ContainsKey(newRPathPos)) {
						// En los caminos secundarios se pueden generar enemigos
